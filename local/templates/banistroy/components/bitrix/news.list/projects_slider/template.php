...
<?if ($arResult["ITEMS"]):?>
<?if ($arParams["TITLE"]):?>
        <h2 class="global-title"><?= $arParams["TITLE"] ?></h2>
    <?endif;?>
    <div class="slider product-slider indent-bt dots-none">
        <?foreach($arResult["ITEMS"] as $arItem):?>
        <?
        $this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_EDIT"));
        $this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
        ?>
        <div class="product-slider__item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
            <div class="product-item">
                ...
                <ul class="product-item__characteristics">
                    ...
                    <li>
                        <span><?=$prop["NAME"]?>: <strong><?=$prop["VALUE"]?></strong></span>
                    </li>
                    <?endforeach;?>
<!--                    <li>
                        <span>Общая площадь: <strong>80 м2</strong></span>
                    </li>-->
                </ul>
    ...
    <?if ($arParams["HIDE_ALL_LINK"]!="Y"):?>
    <div class="btn-panel indent-bt text-right">
        <a href="/projects/" class="btn-small">все проекты</a>
    </div>
    <?endif;?>
<?endif;?>
